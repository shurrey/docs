...
import { ERROR_CODE_EMPTY_METADATA_SUGGESTIONS, ERROR_CODE_FETCH_METADATA_SUGGESTIONS, ERROR_CODE_METADATA_AUTOFILL_TIMEOUT, ERROR_CODE_UNKNOWN, ERROR_CODE_METADATA_PRECONDITION_FAILED, FIELD_IS_EXTERNALLY_OWNED, FIELD_PERMISSIONS_CAN_UPLOAD, FIELD_PERMISSIONS, SUCCESS_CODE_UPDATE_METADATA_TEMPLATE_INSTANCE, SUCCESS_CODE_DELETE_METADATA_TEMPLATE_INSTANCE } from '../../../constants';
...
function useSidebarMetadataFetcher(api, fileId, onError, onSuccess, isFeatureEnabled) {
  ...
  const handleCreateMetadataInstance = React.useCallback(async (templateInstance, successCallback) => {
    await api.getMetadataAPI(false).createMetadataRedesign(file, templateInstance, successCallback, (error, code) => onApiError(error, code, messages.sidebarMetadataEditingErrorContent));
  }, [api, file, onApiError]);
